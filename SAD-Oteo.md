...
```
  ...
  └─────────────────────────────────┬────────────────────────────────┘
      ...
      │ filtrable  │        │ Leaflet +  │         │ (Turbo     │
      │ (Turbo     │        │ Stimulus   │         │ Streams,   │
...
- **`businesses`** — el registro central. `source` (enum `places`/`manual`, ADR-012), `place_id` (**nullable**, índice único parcial; clave estable ADR-006), datos de Places (`name`, `address`, `lat`, `lng`, `phone`, `rating`, `user_rating_count`, `website_uri`, `types jsonb`, `business_status`, `synced_at`), clasificación (`digital_presence` enum, `pos_candidate` bool, `lead_score`), datos propios (`pos_status` enum, `pos_vendor`, `pipeline_stage` enum — incluye `archivado`, ADR-013 —, `comuna` asignada desde la consulta de origen).
